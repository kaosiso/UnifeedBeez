...
export interface Plan {
  ...
  label: string;         // Plan label, e.g., "Your Plan"
  price: string;         // Price string, e.g., "£219.2"
  buttonText: string;    // Button text, e.g., "Upgrade Plan"
  features: string[];    // List of features/add-ons
}
...
// --- Component ---
const BusinessPlanCard: React.FC<BusinessPlanCardProps> = ({ plan }) => {
  return (
    <div className="bg-gradient-to-br from-[#ffffff] via-[#fffcf5] to-[#fef5e0] shadow-sm rounded-2xl border border-gray-300 p-5 max-w-2xl">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <div className="border-2 rounded-lg h-10 w-10 flex items-center justify-center bg-red-500">
            <Image src="icons/luggage-02.svg" alt="Plan Icon" className="h-5 w-auto" />
          </div>

          <h4 className="font-medium capitalize text-lg text-[#323232]">{plan.name}</h4>
          <span className="text-sm font-medium text-[#053d27] border-2 border-[#053d27] bg-[#f2f4f2] px-2 py-0.5 rounded-3xl">{plan.label}</span>
        </div>
      ...
      <div className="mt-2 flex items-end space-x-2">
        <p className="text-2xl font-medium text-[#053d27] mt-2">{plan.price}</p>
